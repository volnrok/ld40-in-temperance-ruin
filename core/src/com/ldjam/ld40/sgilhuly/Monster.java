...
public class Monster extends Creature {
	...
	public static Monster[] MONSTERS = {
			...
			new Monster("Zapper", 14, 14, 24, 12, 7, 70).sound(GameContext.audio.impAppears).loadImage("texture/monsters/imp.png", Palette.RED, Palette.YELLOW).difficulty(25).spell("Zap", 50, Combat.ELEC).resist(Combat.ELEC, 80),
			...
			new Monster("Sneaker", 10, 11, 13, 14, 0, 70).sound(GameContext.audio.ratmanAppears).loadImage("texture/monsters/rat.png", Palette.YELLOW, Palette.ORANGE).difficulty(13).resist(Combat.PHYS, 20).swings(2),
			new Monster("Virulent", 15, 12, 14, 16, 6, 80).sound(GameContext.audio.ratmanAppears).loadImage("texture/monsters/rat.png", Palette.YELLOW, Palette.GREEN).difficulty(16).resist(Combat.PHYS, 20).resist(Combat.POIS, 60).spell("Spit", 40, Combat.POIS),
			new Monster("Hirsute", 17, 12, 14, 20, 12, 80).sound(GameContext.audio.ratmanAppears).loadImage("texture/monsters/rat.png", Palette.YELLOW, Palette.BLUE).difficulty(22).resist(Combat.PHYS, 20).resist(Combat.ICE, 60).spell("Frost", 60, Combat.ICE),
			...
			new Monster("Mimic", 20, 20, 8, 17, 10, 140).sound(GameContext.audio.mimicAppears).loadImage("texture/monsters/mimic.png", 0, 0).difficulty(19).resist(Combat.FIRE, 30).resist(Combat.ELEC, 50).spell("Blast", 20, Combat.FIRE),
			new Monster("Fridge", 24, 24, 11, 17, 13, 150).sound(GameContext.audio.mimicAppears).loadImage("texture/monsters/mimic.png", Palette.ORANGE, Palette.BLUE).difficulty(28).resist(Combat.PHYS, 30).resist(Combat.ICE, 50).spell("Chill", 20, Combat.ICE),
			new Monster("Fusebox", 25, 26, 12, 17, 14, 160).sound(GameContext.audio.mimicAppears).loadImage("texture/monsters/mimic.png", Palette.ORANGE, Palette.YELLOW).difficulty(34).resist(Combat.PHYS, 30).resist(Combat.ELEC, 50).spell("Short", 40, Combat.ELEC),
			new Monster("Barbecue", 26, 27, 13, 17, 15, 170).sound(GameContext.audio.mimicAppears).loadImage("texture/monsters/mimic.png", Palette.ORANGE, Palette.RED).difficulty(37).resist(Combat.PHYS, 30).resist(Combat.FIRE, 50).spell("Char", 40, Combat.FIRE),
			new Monster("Dumpster", 28, 28, 14, 17, 20, 180).sound(GameContext.audio.mimicAppears).loadImage("texture/monsters/mimic.png", Palette.ORANGE, Palette.GREEN).difficulty(46).resist(Combat.PHYS, 30).resist(Combat.POIS, 50).spell("Rot", 60, Combat.POIS),

			new Monster("Gazer", 12, 30, 20, 15, 15, 180).sound(GameContext.audio.beholderAppears).loadImage("texture/monsters/gazer.png", 0, 0).difficulty(40).resist(Combat.DEATH, 60).resist(Combat.PHYS, 40).spell("Death Ray", 70, Combat.DEATH).swings(3),
			new Monster("Observer", 12, 30, 25, 15, 15, 180).sound(GameContext.audio.beholderAppears).loadImage("texture/monsters/gazer.png", Palette.PURPLE, Palette.RED).difficulty(43).resist(Combat.DEATH, 30).resist(Combat.FIRE, 60).resist(Combat.PHYS, 40).spell("Heat Ray", 70, Combat.FIRE).swings(3),
			new Monster("Watcher", 12, 35, 20, 15, 15, 180).sound(GameContext.audio.beholderAppears).loadImage("texture/monsters/gazer.png", Palette.PURPLE, Palette.BLUE).difficulty(49).resist(Combat.DEATH, 30).resist(Combat.ICE, 40).spell("Ice Ray", 70, Combat.ICE).swings(3),
			new Monster("Beholder", 17, 30, 20, 15, 25, 180).sound(GameContext.audio.beholderAppears).loadImage("texture/monsters/gazer.png", Palette.PURPLE, Palette.GREEN).difficulty(52).resist(Combat.DEATH, 30).resist(Combat.POIS, 40).spell("Acid Ray", 70, Combat.POIS).swings(3),
	};
